...
angular.module('admin').controller('NewsletterCtrl', ["$scope", "$http", "MeuService", function ($scope, $http, MeuService) {
	...
	$http({method: 'GET',url: $scope.teste+'services/fornecedor'}).then(function successCallback(response) {
	...
	$http({method: 'GET',url: $scope.teste+'services/vendedor'}).then(function successCallback(response) {
	...
	$http({method: 'GET',url: $scope.teste+'services/telemarketing'}).then(function successCallback(response) {
	...
	$scope.pesquisar = function (search) {
		...
		if (search.length > 3) {
			loadingOn();
			$http({method: 'GET',url: $scope.teste+'services/cliente/search/'+search}).then(function successCallback(response) {
	...
	$scope.pesquisarPedido = function () {
		...
		loadingOn();
		$http({method: 'GET',url: $scope.teste+'services/pedido/'+$scope.pedido.pedido}).then(function successCallback(response) {
	...
	$scope.cadastrar = function() {
		...
		loadingOn();
		$http({method: 'POST',url: $scope.teste+'services/pedido',data: {pedidox:$scope.pedido}}).then(function successCallback(response) {
	...
	$scope.pesquisarProduto = function (search) {
		...
		if (search.length > 2) {
			loadingOn();
			$http({method: 'GET',url: $scope.teste+'services/produto/search/'+search}).then(function successCallback(response) {
	...
	$scope.adicionarItem = function() {
		...
		if (!contains($scope.itens,$scope.pItem)) {
			...
			loadingOn();
			$http({method: 'POST',url: $scope.teste+'services/pedidoItem',data: {pItemx:$scope.pItem}}).then(function successCallback(response) {
	...
	$scope.carregaFornecedor = function() {
		$http({method: 'GET',url: $scope.teste+'services/fornecedor/'+$scope.pedido.fornecedor}).then(function successCallback(response) {
	...
	$scope.baixar = function() {
		...
		loadingOn();
		$http({method: 'POST',url: $scope.teste+'services/baixa',data: {pedidox:$scope.pedido}}).then(function successCallback(response) {
	...
	$scope.excluir = function(item) {
		...
		loadingOn();
		$http({method: 'DELETE',url: $scope.teste+'services/pedidoItem/'+item.pedido+'/'+item.produto}).then(function successCallback(response) {
			$scope.pesquisarPedidoItem();
			loadingOff();
		}, function errorCallback(response) {
			console.log('Ocorreu um erro!');
	...
	$scope.pesquisarPedidoItem = function () {
		$http({method: 'GET',url: $scope.teste+'services/pedidoItem/'+$scope.pedido.pedido}).then(function successCallback(response) {
			if (response.data) {
				console.log(response.data);
				$scope.pedidoItem = true;
